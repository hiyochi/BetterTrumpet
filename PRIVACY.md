...
* Until an explicit telemetry choice has been stored, including on upgraded profiles
* When the user has opted out (`IsTelemetryEnabled = false`)

Microsoft Store / MSIX builds do not start the GitHub updater. Sentry remains independent
of the updater and follows the same telemetry-consent rules in every distribution channel.
