...
- Per-device hotkeys in the React settings: the shortcuts page gained a `Device shortcuts` section with one recordable hotkey per playback device. Stored in `AppSettings` as `DeviceHotkey:<deviceId>` via `GetDeviceHotkey`/`SetDeviceHotkey` (persist + individual `HotkeyManager` register/unregister on set). The web UI ships the device list from `App.AudioDeviceManager.Devices` as `collections.deviceHotkeys` (ids `device:<id>`); `BuildHotkeys` keeps ONLY the six global shortcuts, so the two collections never overlap. Triggering lives in `App.xaml.cs`: `RegisterDeviceHotkeys()` registers persisted per-device hotkeys at startup and `OnDeviceHotkeyPressed` matches the pressed combo against each device to set it as the default. New resx keys `WebSettingsDeviceShortcuts`/`WebSettingsDeviceShortcutDesc` (EN/FR + Designer.cs) feed the `deviceShortcuts`/`deviceShortcutDesc` web labels. A device with no hotkey sends an empty `value` so the web clear button stays hidden, matching the global shortcuts.
...
- Settings UI visual language: `designglobal.md` at the repo root defines the "Sage Glass Control Surfaces" DA applied to the React settings. Key rules: near-black outer shell (`#050706`) with a quiet Wafer-inspired sidebar (248px ↔ 64px via a header toggle; spec in `sidebar-design.md`), an Ink dither field on the main surface (lab variant 07 from `dither-field.tsx`: periwinkle ridge + bloom, canvas 2D, no WebGL; paused when eco mode or reduced-motion), a strict radius scale (`14px` controls / `18px` cards / `24px` shell), one accent color (violet `#9b7bea`) reserved for actions/active states, dark-only palette, and `prefers-reduced-motion`/`prefers-reduced-transparency` fallbacks. Cards use translucent glass (`rgba(22,18,32,.66)` + `backdrop-filter: blur(14px)` + hairline `rgba(255,255,255,.10)` border). Do not reintroduce per-element radii, a second accent, or the WebGL `Scanner` component.
- Theme preset curation + per-theme opacity (2026-08-21): `ThemeRegistry` went from 30 presets in 6 categories to 12 fully-crafted ones in 5 categories (`Minimal`, `Apps`, `Editors`, `Atmosphere`, `Accessibility`). Every preset now carries a `WindowBackgroundOpacity` personality applied by `ApplyTheme` (Midnight Studio/Aurora 0.45 glassy → High Contrast 1.0 solid; "Default (Windows Accent)" leaves the user opacity untouched). `ColorTheme.WindowBackgroundOpacity` is nullable, serialized as `Opacity` in `ToJson`/`FromJson`, captured by `SaveCustomTheme`, and clamped 0.05–1.0 on import; the web opacity slider refreshes after `themeSelect` because `HandleAction` ends with `PostState()`. Classic settings category chips were rewired to the new category strings (`CategoryBrands_Click`→"Apps", `CategoryDev_Click`→"Editors", new `CategoryAtmosphere_Click`; Retro/Nature chips removed); resx keys `PresetCategoryAtmosphere` added EN/FR. Users who had a removed preset active keep their applied colors (stored in settings); only the selection highlight disappears.
- Web Appearance page is now preset-chips + live editor (2026-08-21): presets render as a horizontal wrap of compact chips (`themeFlow`/`themeChip`, left-to-right, check mark on active), "Custom colors" is always visible directly below as the editor of the active theme (no `useCustomSliderColors` toggle in the web UI — tweaking any of the 7 channels auto-enables custom mode host-side via `EarTrumpetColorsSettingsPageViewModel.EnsureCustomColorsActive()`, called from every channel setter and mirroring `ApplyPickedColor`; it also takes the refs backup so `RestoreOriginalThemeRefs` stays valid), and the Dynamic album art section moved to the bottom of the page. New resx key `WebSettingsCustomColorsTweakHint`. The classic window keeps its explicit toggle; the web writes still flow through the same VM setters, so both UIs stay consistent.
- QuickTrumpet/rules/themes web settings redesign (2026-08-21): `ProfilesPage` is now presets-first — one accordion row per profile (name + device/app meta from enriched `BuildProfileDetails`, inline recordable hotkey, primary Apply button that implicitly selects via `profileApply`), expanded panel holds Apps-only switch, Rename (new bridge action `profileRename { index, name }` → `EarTrumpetVolumeProfilesSettingsPageViewModel.RenameSelectedProfile` → existing `VolumeProfileService.RenameProfile`; slug stays stable for CLI), export/delete; save bar (name + all-devices + import + Save) sits above the list and doubles as the empty state with hint key `emptyProfilesHint`. `RulesPage` rules are expandable accordion rows: collapsed line shows name + summary chips (muted/focus-lost/`49% · mode`); expanded panel stacks the switches and volume dropdown/slider vertically; folder defaults use the same pattern. Theme presets are now a compact single-column list (`themeRow`, small swatch pills, check mark for active, delete for custom) instead of the old 160px swatch-card grid. New resx keys: `SettingsQuickTrumpetAppsOnlyDesc`, `WebSettingsSave`, `WebSettingsRename`, `WebSettingsQuickTrumpetEmptyHint`, `FolderVolumeRulesHeaderDesc` (EN+FR+Designer.cs). Griffel gotcha: `borderColor` alone does not type-check in makeStyles — use full `border`.
- CSS in the React settings must be delivered by importing it in TS/TSX (`import "./polish.css"` in `App.tsx`); Vite bundles it into the hashed `index-*.css`. Do NOT hand-edit `SettingsWeb/index.html` (Vite regenerates it every build) and do NOT drop CSS in `public/` for manual `<link>` tags (the file is copied to `dist/` but never loaded). Sidebar polish styles live in `src/polish.css` and are applied via extra class names like `sidebar-polished`. The sidebar itself: `#050706` rail with no vertical border, 8px gap + 18px radius so it reads as a floating panel, 42px nav rows, hover `rgba(255,255,255,.06)`, active state a neutral capsule (`rgba(255,255,255,.10)` + inset hairline — not the violet accent), Lucide icons via `@animateicons/react`, footer pinned with `margin-top: auto` (Classic settings + GitHub). Collapsed mode centers icons and keeps `title` tooltips. Narrow windows (≤680px) hide the rail and open a 248px drawer under a 58px mobile bar (Escape / backdrop / item click close). `WebSettingsWindow` cache-busts the virtual-host page load with a `?v=<index.html mtime ticks>` query so WebView2 never serves a stale `index.html` after a rebuild; hashed asset names handle the rest.
- Web settings loading: there is no WPF loading overlay anymore. `WebSettingsWindow` enables native DWM acrylic (`AccentPolicyLibrary.EnableAcrylic` on the WebView2 host, tint `0xC8` over the theme shell color) and shows a branded XAML splash (`LoadingSplash`: pulsing logo + wordmark + indeterminate bar on `#050706`, `IsHitTestVisible=False`) that covers the WebView2 cold-start phase — without it the window paints pure black because `Background=Transparent` + `AllowsTransparency=False`. The splash fades out (240 ms) when the React app sends its `rendered` message and is force-hidden by `ShowLoadError` (which also now sets `ErrorOverlay.Visible`; it previously set Collapsed — fixed). Inside the web UI a shimmer skeleton with a pulsing logo (`sage-pulse` in styles.css, reduced-motion safe) mirrors the final layout until the first `state`, then AnimatePresence crossfades skeleton → shell (blur+fade) and each page change animates in (opacity + 10px rise). Respect these phases if touching load flow.
- Live performance status: `WebSettingsWindow` runs a 1 s `DispatcherTimer` (`StartStatusTimer`, stopped on `Closed`) posting `{type:"status", data:{effectivePeakMeterFps, ecoModeActive}}`; App.tsx patches `payload.status` from it. The Performance page shows the effective FPS as a chip row ("Effective rate") inside Peak meter instead of a floating text. New resx keys `WebSettingsEffectiveRate`/`WebSettingsEffectiveRateHint` (EN+FR+Designer.cs).
- Deep search + calm surfaces (2026-08-22): the web settings behave like a native app — `user-select: none` globally (inputs/textarea keep selection) and NO hover effects on setting/list/accordion rows or section cards (`.setting-row-polished`, `.list-row-polished`, `.acc-header-polished`, `.section-polished:hover` were removed from polish.css; accordion headers keep only `cursor: pointer`). Do not reintroduce row hovers or translateX micro-motions. The sidebar search is now a real engine: every `Section` takes an `anchor` prop (stable ids like `"startup"`, `"focusLost"`, `"themes"`), `SEARCH_SPECS` in App.tsx indexes all sections with bilingual synonym hints, plus dynamic entries (devices, presets, rules, folders, themes). Matching folds accents per-character (length-preserving so highlight offsets map 1:1 via `HighlightedLabel`), scores word-prefix > substring > gap-penalized fuzzy subsequence, and requires AND across space-separated tokens. Precision guards (added after "album art" returned noise): tokens shorter than 4 chars match only by literal prefix/substring (no fuzzy), fuzzy subsequences must fit a span ≤ token length × 4 and clear `MIN_TOKEN_SCORE` 24, every kept token clears that threshold, exact multi-word phrases get +80, all-tokens-in-title gets +40. Results render as a dropdown under the search input (top 9, highlighted matches, page meta, `↵` kbd hint) with ArrowUp/Down/Enter/Escape; selecting navigates to the page and scrolls the anchor into view with a `.search-flash` outline pulse. Search styles live under `search-polished`/`searchResults` classes; Griffel gotcha repeated: never use bare `borderColor` in makeStyles.
- The React settings also expose the master features added 2026-08: focus-lost volume (enable, attenuation %, fade ms, scope all/selected apps), device-change notification, and the per-app `focusLost` switch on app rules (bridge key `focusLost` on `appRuleUpdate`, stored via `SetAppFocusLost`). New settings keys flow: add to `types.ts` `SettingKey`, `ApplySetting` switch in `WebSettingsWindow.xaml.cs`, `values` in `PostState()`, and labels via resx-backed entries in the `labels` dictionary.
...
- Transparent WPF overlays above `SettingsWebView` must set `IsHitTestVisible="False"` unless they contain active controls. A transparent loading grid otherwise makes the rendered React UI look ready while intercepting every click. (The WPF loading overlay was removed; the React skeleton owns loading now.)
...
- Fluid app-row insertion: when a session is added at runtime (`AnimateOnLoad=true`), `AppItemView` also grows its `ListViewItem` container from 0 to its natural height over the same 220 ms ease-out as the content fade/8px slide, so rows below shift smoothly instead of popping. Height is restored to auto on completion and cleared again in `ResetTransientVisualState` for recycled containers. The implicit flyout `ListViewItem` style has no MinHeight/Height, which is what makes the height animation safe.
- Volume slider thumb offset and animation polish (ported from `xammen/fix-position-pr47`, originally uncommitted worktree state): `SetPositionByControlPoint` compensates thumb width so the thumb center aligns with the click position. Track clicks use a short, time-based cubic ease-out that continues after release and retargets from the currently rendered value; as soon as a held track click moves, it uses a 50 ms handoff into the same immediate, tick-enabled mode as a direct thumb drag, preventing a jump when the click starts far from the current value. Animated glides emit the same throttled tick feedback as drags (`_isAnimatingValue` included in the `OnValueChanged` tick condition), so a click-glide is audible like a drag. During animation, the template `Track` receives the fractional visual value every frame while the integer-bound audio source is written only when its percentage changes, avoiding unnecessary WASAPI work and preserving smooth motion. `Unloaded` calls `CompleteVolumeAnimation` before stopping `CompositionTarget.Rendering`, so closing the flyout cannot leave `Value` stuck partway. This also resolved an intermittent drag-resets-to-original-position issue caused by imprecise `Value` writes interacting with Lock rule enforcement via `ApplyRuleToApp`.
...
17. Track clicks may use smooth animation (`animate: true`) for the volume slider. Keep the short time-based animation running after `MouseUp`/`TouchUp` so a quick click does not pop to the target. If the pointer moves while the track click is held, use only a 50 ms handoff before switching to direct drag mode so a far-away click cannot jump, while `OnValueChanged` still emits the normal tick feedback — but if the pointer moves within that same 50 ms window of the press, skip the handoff entirely and drag directly: an immediate move is a drag intent, and running the catch-up slide then reads as a glitch. The slider's integer `Value` binding must not be used as the only per-frame visual source: update the template `Track.Value` fractionally and commit the audio binding only at percentage changes. `Unloaded` must call `CompleteVolumeAnimation` before the render subscription is stopped, otherwise the flyout can unload mid-animation and leave `Value` stuck partway.
18. The volume slider's fractional track motion is driven by a WPF `DoubleAnimation` clock on `PART_Track.Value`; `CompositionTarget.Rendering` should only synchronize integer audio commits, ticks, and peak meters. Do not restore manual per-frame `Track.Value` writes unless profiling proves the WPF clock is insufficient. During the 50 ms click-to-drag handoff, preserve the original deadline when retargeting from `MouseMove`; restarting the full handoff duration on every move makes the thumb perpetually lag behind a moving cursor. Once the handoff budget is spent (remaining ≤ 16 ms), clear `_isDragHandoffAnimating` and finish as a plain direct drag instead of restarting micro-animations on every MouseMove.
19. `VolumeSlider.SetPositionByControlPoint` maps `percent` back onto the value range with `Bound(Minimum + (Maximum - Minimum) * percent)`. Keep the `Minimum +` term even though `Minimum` is 0 today: it is only 0 because the base `Slider` style in `App.xaml` sets `Minimum="0"` and no usage overrides it. Without the term, a non-zero `Minimum` turns the low end of the track into a dead zone and makes the top of the range unreachable, and `Bound` clamps the underflow so it fails silently rather than throwing.
