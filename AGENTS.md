...
- Web settings loading: there is no WPF loading overlay anymore. `WebSettingsWindow` enables native DWM acrylic (`AccentPolicyLibrary.EnableAcrylic` on the WebView2 host, tint `0xC8` over the theme shell color) and the React app shows a shimmer skeleton that mirrors the final layout (`LoadingSkeleton` in `App.tsx`) until the first `state` message arrives. `DefaultBackgroundColor="0"` keeps the WebView transparent before first paint, so the acrylic shows through instead of a white or progress-bar flash. Keep this pattern: the web UI owns all loading/empty/error states.
