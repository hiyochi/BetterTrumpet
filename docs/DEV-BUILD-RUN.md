...
& 'C:\Program Files\dotnet\dotnet.exe' build `
    ...
    -p:GitVersion_MajorMinorPatch=3.2.3 `
    -p:GitVersion_InformationalVersion=3.2.3-dev `
    -p:Version=3.2.3-dev `
    -p:FileVersion=3.2.3.0 `
    -p:AssemblyVersion=3.2.3.0 `
    -p:InformationalVersion=3.2.3-dev `
...
Les propriétés `GenerateAssemblyInfo` sont importantes dans ce mode. Sans elles, WPF peut chercher `BetterTrumpet, Version=3.2.3.0` alors que l'assembly généré n'a pas cette version, puis quitter avec une `FileNotFoundException` pendant `App.InitializeComponent()`.
